...
void histogram(float *values, int size);
...
/**********************************************************************/
/* This function prints out the five related stats of the array.
 * 
 *   values: the array of data
 *   size: the number of data values
 */
...
void histogram(float *values, int size) {
  ...
  int *buckets = (int *)malloc(sizeof(int)*11); // the array of buckets
  
  for(i = 0; i < 11; i++)
    buckets[i] = 0;

  for(i = 0; i < size; i++)
    buckets[((int) values[i])/10]++;
